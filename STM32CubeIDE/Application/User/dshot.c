...
void DShot_TimerInit(DShotType dshotType, TIM_HandleTypeDef *tim, uint32_t channel){
	...
	switch (dshotType){
	case DSHOT150:
		prescaler = (SystemCoreClock / 150000) - 1;
		break;
	case DSHOT300:
		prescaler = (SystemCoreClock / 300000) - 1;
		break;
	case DSHOT600:
		prescaler = (SystemCoreClock / 600000) - 1;
		break;
	case DSHOT1200:
		prescaler = (SystemCoreClock / 1200000) - 1;
		break;
	default:
		break;
	}
...
static void DShot_PrepareDmaBuffer(uint16_t dshotPacket){
    for (uint8_t i=0; i<DSHOT_BIT_COUNT; i++){
        if (dshotPacket & (1 << (15 - i))){
        	...
        	dshotDmaBuffer[i] = (uint16_t)(0.3f * (float)pwmHandler.Tim->Instance->ARR);;  // logic 0: ~37.5% of ARR
        }
    }
}
